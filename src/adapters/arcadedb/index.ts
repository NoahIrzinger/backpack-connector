...
export class ArcadeDBAdapter implements ConnectorAdapter {
  ...
  async dropDatabase(database: string): Promise<void> {
    this.nodeTypeCache.delete(database);
  ...
  async getLastOrdinal(database: string, graph: string, branch: string): Promise<number> {
    try {
      const rows = await this.client.query(
        database,
        "SELECT last_ordinal FROM BackpackState WHERE bk_graph = :graph AND bk_branch = :branch",
        { graph, branch },
      );
  ...
  async setLastOrdinal(database: string, graph: string, branch: string, ordinal: number): Promise<void> {
    await this.client.command(
      database,
      "UPDATE BackpackState SET last_ordinal = :ordinal, projected_at = :projected_at, bk_graph = :graph, bk_branch = :branch UPSERT WHERE bk_graph = :graph AND bk_branch = :branch",
      { ordinal, projected_at: new Date().toISOString(), graph, branch },
    );
  }

  async applyEvent(event: GraphEvent, database: string, graph: string, branch: string): Promise<void> {
    const result = translateEvent(event, graph, branch);
    if (!result) return;
    ...
    if (result.kind === "cmds") {
      ...
      if (event.op === "node.add") {
        this.cacheNodeType(database, event.node.id, sanitizeIdent(event.node.type));
      }
      return;
    }

    if (result.kind === "needs-node-lookup") {
      const typeSafe = await this.resolveNodeType(database, result.bkId);
      if (!typeSafe) {
      ...
      if (event.op === "node.update") {
        ...
        this.nodeTypeCache.get(database)?.delete(result.bkId);
      }
      return;
    }
    ...
    if (result.kind === "needs-edge-lookup") {
      ...
      const sourceType = await this.resolveNodeType(database, edge.sourceId);
      const targetType = await this.resolveNodeType(database, edge.targetId);
      if (!sourceType || !targetType) {
        process.stderr.write(
          `Warning: edge.add skipped — endpoint type unknown (${edge.sourceId} → ${edge.targetId})\n`,
        );
        return;
      }
      const cmds = translateEdgeAdd(edge, sourceType, targetType, graph, branch);
      for (const cmd of cmds) await this.client.command(database, cmd.sql, cmd.params);
      return;
    }
    ...
    if (result.kind === "needs-edge-type-lookup") {
      ...
      for (const typeSafe of edgeTypes) {
        const rows = await this.client.query(
          database,
          `SELECT bk_id FROM ${typeSafe} WHERE bk_id = :bk_id`,
          { bk_id: result.edgeId },
        );
        if (rows.length > 0) {
          const cmd = translateEdgeRemove(
            { v: 1, ts: new Date().toISOString(), op: "edge.remove", id: result.edgeId },
            typeSafe,
          );
          await this.client.command(database, cmd.sql, cmd.params);
          return;
      ...
      process.stderr.write(`Warning: edge.remove skipped — edge ${result.edgeId} not found in any edge type\n`);
    }
  }
  ...
  async getSchema(database: string): Promise<ConnectorSchema> {
    ...
    const vertexTypes = types
      .filter((t) => t.type === "vertex")
      .map((t) => ({
        name: String(t.name),
        properties: Object.keys((t.properties as Record<string, unknown>) ?? {}).filter(
          (p) => !p.startsWith("@"),
        ),
      }));

    const edgeTypes = types
      .filter((t) => t.type === "edge")
      .map((t) => ({ name: String(t.name) }));

    const stateRows = await this.client.query(
      database,
      "SELECT bk_graph, bk_branch, last_ordinal, projected_at FROM BackpackState",
  ...
  private cacheNodeType(database: string, bkId: string, typeSafe: string): void {
    let dbCache = this.nodeTypeCache.get(database);
    if (!dbCache) { dbCache = new Map(); this.nodeTypeCache.set(database, dbCache); }
    dbCache.set(bkId, typeSafe);
  }

  private async resolveNodeType(database: string, bkId: string): Promise<string | null> {
    const cached = this.nodeTypeCache.get(database)?.get(bkId);
    if (cached) return cached;
    try {
      const rows = await this.client.query(
        database,
        "SELECT node_type FROM BackpackIndex WHERE bk_id = :bk_id",
        { bk_id: bkId },
      );
      const raw = rows[0]?.node_type;
      if (typeof raw !== "string") return null;
      const typeSafe = sanitizeIdent(raw); // re-sanitize DB-sourced value before SQL injection
      this.cacheNodeType(database, bkId, typeSafe);
      return typeSafe;
    } catch {
      return null;
    }
  ...
  private async warmNodeTypeCache(database: string): Promise<void> {
    try {
      const rows = await this.client.query(
        database,
        "SELECT bk_id, node_type FROM BackpackIndex LIMIT 100000",
      );
      for (const row of rows) {
        if (typeof row.bk_id === "string" && typeof row.node_type === "string") {
          this.cacheNodeType(database, row.bk_id, row.node_type);
        }
      }
    } catch {
