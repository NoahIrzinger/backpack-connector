...
async function detectTypeDrift(
  ...
  for (const [, nodes] of byName) {
    ...
    signals.push({
      ...
      description: `"${nodes[0].name}" appears as ${types.map((t) => `"${t}"`).join(" and ")} across ${graphs.join(", ")}. Inconsistent typing breaks cross-graph traversal.\n→ Action: Pick one type and retype the others. Use backpack_update_node to retype the inconsistent nodes, then re-project.`,
    ...
    try {
      ...
      for (const r of rows) {
        ...
        signals.push({
          ...
          description: `"${r.name}" (${r.bk_type}) connects to ${degree} other entities — ${(degree / (avg || 1)).toFixed(1)}× the graph average of ${avg.toFixed(1)}. Hub nodes often represent over-generalized concepts.\n→ Action: Consider whether "${r.name}" should be split into more specific types. Run the "Everything connected to a node" template in the Query panel to inspect its connections first.`,
    ...
    try {
      ...
      for (const r of rows) {
        ...
        signals.push({
          ...
          description: `"${r.name}" (${r.type}) connects to entities from ${connectedGraphs.join(", ")} and more.\n→ Action: This node is the structural link between your graphs. Make sure it's typed consistently and has rich properties — it will be the anchor for synthesis.`,
          evidenceNodeIds: [String(r.id ?? "")],
          evidenceDocIds: [],
...
async function detectCrossGraphStructural(
  ...
  for (const [, nodes] of byLabel) {
    ...
    signals.push({
      ...
      description: `"${nodes[0].name}" exists in ${graphs.join(", ")}${hasDrift ? ` with inconsistent types (${types.join(", ")})` : ""}. Confidence: ${Math.round(confidence * 100)}%.\n→ Action: ${confidence >= 0.8 ? `High confidence — these are likely the same entity. Remove the duplicate from one graph and keep the richer version.` : `Medium confidence — compare properties in both graphs before deciding. Use connector_query to inspect both nodes side by side.`}`,
      evidenceNodeIds: nodes.map((n) => n.bkId),
      evidenceDocIds: [],
      graphNames: graphs,
