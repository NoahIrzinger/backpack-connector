...
describe("translateNodeAdd", () => {
  it("produces UPSERT sql with w_bk_id as WHERE param", () => {
    const cmd = translateNodeAdd(node, "my-graph", "main");
  ...
  it("stores bk_graph and bk_branch", () => {
    const cmd = translateNodeAdd(node, "my-graph", "main");
  ...
  it("sanitizes property keys", () => {
    ...
    const cmd = translateNodeAdd(nodeWithSpace, "g", "main");
  ...
  it("w_bk_id is separate from p_bk_id so property named bk_id cannot overwrite WHERE param", () => {
    ...
    const cmd = translateNodeAdd(nodeWithBkId, "g", "main");
    expect(cmd.params!.w_bk_id).toBe("n_abc123");
...
describe("translateIndexUpsert", () => {
  it("upserts into BackpackIndex with bk_id and node_type", () => {
    const cmd = translateIndexUpsert("n_abc123", "Platform");
...
describe("translateEdgeAdd", () => {
  it("produces DELETE then CREATE EDGE commands", () => {
    const cmds = translateEdgeAdd(edge, "Platform", "API", "my-graph", "main");
  ...
  it("references source and target by sanitized type", () => {
    const cmds = translateEdgeAdd(edge, "Platform", "API", "g", "main");
...
describe("translateEvent", () => {
  it("returns cmds kind for node.add", () => {
    const result = translateEvent({ v: 1, ts: "2026-01-01T00:00:00.000Z", op: "node.add", node }, "g", "main");
    expect(result?.kind).toBe("cmds");
    expect((result as any).nodeType).toBe("Platform");
  });

  it("returns needs-node-lookup for node.update", () => {
    const result = translateEvent({ v: 1, ts: "t", op: "node.update", id: "n_abc", properties: { name: "New" } }, "g", "main");
  ...
  it("returns needs-node-lookup for node.remove", () => {
    const result = translateEvent({ v: 1, ts: "t", op: "node.remove", id: "n_abc" }, "g", "main");
    expect(result?.kind).toBe("needs-node-lookup");
    expect((result as any).bkId).toBe("n_abc");
  });

  it("returns needs-edge-lookup for edge.add", () => {
    const result = translateEvent({ v: 1, ts: "t", op: "edge.add", edge }, "g", "main");
  ...
  it("returns null for snapshot.label and metadata.update", () => {
    expect(translateEvent({ v: 1, ts: "t", op: "snapshot.label" }, "g", "main")).toBeNull();
    expect(translateEvent({ v: 1, ts: "t", op: "metadata.update", patch: {} }, "g", "main")).toBeNull();
  });
});
